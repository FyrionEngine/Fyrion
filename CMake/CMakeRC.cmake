...
if(_CMRC_GENERATE_MODE)
    ...
    string(REGEX REPLACE "(..)" "0x\\1," chars "${bytes}")
...
#    endif()
    string(CONFIGURE [[
        namespace { const unsigned char file_array[] = { @chars@ 0 }; }
        namespace cmrc { namespace @NAMESPACE@ { namespace res_chars {
        extern const unsigned char* const @SYMBOL@_begin = file_array;
        extern const unsigned char* const @SYMBOL@_end = file_array + @n_bytes@;
        }}}
    ]] code)
...
class file {
    const unsigned char* _begin = nullptr;
    const unsigned char* _end = nullptr;
...
public:
    ...
    file_data(const unsigned char* b, const unsigned char* e) : begin_ptr(b), end_ptr(e) {}
};
...
public:
    ...
    file_or_directory* add_file(std::string name, const unsigned char* begin, const unsigned char* end) & {
...
function(cmrc_add_resources name)
    ...
    foreach(input IN LISTS ARG_UNPARSED_ARGUMENTS)
        ...
        if(relpath MATCHES "^\\.\\.")
            ...
            message(SEND_ERROR "Cannot add file '${abs_in}': File must be in a subdirectory of ${ARG_WHENCE}")
            continue()
        endif()
        ...
        set_property(TARGET ${name} APPEND PROPERTY CMRC_EXTERN_DECLS
            "// Pointers to ${input}"
            "extern const unsigned char* const ${sym}_begin\;"
            "extern const unsigned char* const ${sym}_end\;"
            )
